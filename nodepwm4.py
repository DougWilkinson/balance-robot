...
def balance():
    global stopangle,pausecount,pausemax,tangle,cx,cy,cf
    cx = 0.5
    cy = 0.5
    gangle = 0.0
    start = time.ticks_us()
    ...
    while abs(gangle) < stopangle and spin() :  # give up if inclination angle >=45 degrees
        angle  = imu.pitch()
        rate   = imu.get_gy()
        ...
        #tangle = 1 - cf*(cy-.5)
        tangle = constrain(1 - cf*(cy-.5),-MAX_ANGLE,+MAX_ANGLE)
