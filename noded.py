...
from nemadouble import Stepper
...
def balance():
    ...
    while abs(gangle) < 45 and spin() :  # give up if inclination angle >=45 degrees
        ...
        #tangle = 1 - cf*(cy-.5)
        tangle = 1 - speedcontrol( cf*(cy-.5), fspeed)
        # stability control
...
def main():
    while spin():
        ...
        tim.init(freq=tpd, mode=machine.Timer.PERIODIC, callback=issr) #start interrupt routine
        balance()
